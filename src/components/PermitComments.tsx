...
import {
  usePermitComments,
  useAddPermitComment,
  useDeletePermitComment,
  type CommentTier,
  type PermitComment,
} from '@/hooks/usePermitComments';
import { cn } from '@/lib/utils';

/**
 * PermitComments — list + composer for the three-tier comment model
 * (spec: comment-visibility-tiers.md). Work Permits only.
 *
...
export function PermitComments({ permitId }: { permitId: string }) {
  ...
  const { data: comments = [], isLoading } = usePermitComments(permitId);
  const addComment = useAddPermitComment();
  const deleteComment = useDeletePermitComment();
  ...
  const handleSubmit = () => {
    if (!canSubmit) return;
    addComment.mutate(
      { permitId, tier: effectiveTier, body },
      {
        onSuccess: () => {
          setBody('');
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Discuss this permit. Each comment has a visibility tier — the server decides
          who can see it.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3 border-t pt-4">
          <div className="space-y-2">
            <Label htmlFor="permit-comment-body">Add a comment</Label>
            <Textarea
              id="permit-comment-body"
