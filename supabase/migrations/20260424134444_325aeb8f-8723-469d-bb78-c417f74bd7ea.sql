...
BEGIN
  FOR _gp IN SELECT * FROM public.gate_passes LOOP
    ...
    FOREACH _role IN ARRAY _roles LOOP
      ...
      _extra := '{}'::jsonb;
      IF _role = 'security' AND (_j ->> 'security_cctv_confirmed') IS NOT NULL THEN
      ...
      INSERT INTO public.gate_pass_approvals (
        ...
        comments, signature, auth_method, extra
      ) VALUES (
        _gp.id, _role, 'approved',
        _approver_name, _date,
        _comments, _signature, 'password', _extra
      )
